...
//
//	vertex_offset->at(mesh_id).at(vertex_id).first = Vector3(vertex_offset->at(mesh_id).at(vertex_id).first) + Vector3(position_amount_of_change_local) / ALPcollider_ptr->get_collptr().lock()->Wscale;
//	// transform‚Évertex‚Ì‰e‹¿‚ð“ü‚ê‚é
...
//
//	ALPcollider_ptr->get_collptr().lock()->Wposition = ALPcollider_ptr->transform_gameobject.position;
//	ALPcollider_ptr->get_collptr().lock()->Worientation = ALPcollider_ptr->transform_gameobject.orientation;
//}
